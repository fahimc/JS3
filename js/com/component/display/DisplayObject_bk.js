...
function DisplayObject() {
	...
	// public methods:
	       public.addChild = function(child) 
			...
			public.startDrag= function(auto,w,h,xx,yy)
			{
					this.dragHandler = new DragHandler();
					this.dragHandler.attach(this,auto,0,0,w,h,xx);
					if(auto==true)
					{
						 var evObj = document.createEvent('MouseEvents');
		  				evObj.initEvent( MouseEvent.MOUSE_DOWN.name, true, false );
						this.dragHandler._dragBegin(evObj);
					}
			...
			public.dragPause=function(boo)
			{
				if(this.dragHandler)
				{
					this.dragHandler.setPause(boo);
				}
			}
			...
			public.setScrollbar=function(value)
				{
					...
					this.scrollhandle.startDrag(false,0,sb.getHeight()-sh.getHeight(),this.scrollhandle.getX());
					...
					function onScrollMouseUp(e)
					{
						...
						stage.removeEventListener(MouseEvent.MOUSE_MOVE,onHandleMove);
						stage.addEventListener(MouseEvent.MOUSE_MOVE,mouseUpDate);
						//stage.removeEventListener(MouseEvent.MOUSE_DOWN,falsefunc);
						//document.onmousemove = mosueUpDate;
  						//document.onmouseup = null;
  						//document.onmousedown = null;  
